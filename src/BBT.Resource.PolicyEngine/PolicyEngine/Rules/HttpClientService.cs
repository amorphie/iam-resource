...
public class HttpClientService
{
    ...
    private readonly HashSet<string> _bypassHeaders = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    private void AddHeaders(HttpRequestMessage request, dynamic headers, ISpan span)
    {
        ...
        {
            span.CaptureErrorLog(new ErrorLog($"An error occurred during header serialization. Error: {ex.Message}"),
                null, ex);
            return;
        }
        ...
        foreach (var header in headerDic)
        {
            ...
            {
                span.CaptureErrorLog(new ErrorLog($"Failed to add header: {header.Key}. Error: {ex.Message}"), null,
                    ex);
            }
        }
    }
